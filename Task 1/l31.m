...
for epoch = 1:nEpochs
    for n = 1:N
        ...
        for k = 1:K
            for h = 1:H+1
                al = eta*(O(1,k)-Y(iporder(n),k));
                v(k,h) = v(k,h) - al*Hl(1,h);
            end
        end
        ...
        for h = 2:H+1
            for j = 1:D+1
                sum_val = 0;
                for k = 1:K
                    sum_val = sum_val + ((O(1,k) - Y(iporder(n),k))*v(k,h));
                end
                w(h-1,j) = w(h-1,j) - eta*sum_val*Hl(1,h)*(1-Hl(1,h))*X1(iporder(n),j);
            end
        end
        % ---------
    end
    ...
    for i = 1:N
        for j = 1:K
            terror = terror-Y(i,j)*log(ydash(i,j));
        end
    end
    % ---------
